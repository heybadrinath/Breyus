...
import { NotificationPreferences, AINotificationPreferences } from './user.schema';
...
export class UsersController {
    ...
    // AI Buddy notification preferences endpoints (Settings Page)

    @Get('ai-notification-preferences')
    async getAINotificationPreferences(@Res() response: Response): Promise<void> {
        try {
            const accountToken = response.req.signedCookies['account'];
            if (!accountToken) {
                response.status(401).json({
            ...
            const userId = this.authService.verifyAccountToken(accountToken);
            const preferences = await this.usersService.getAINotificationPreferences(userId);
    ...
    @Put('ai-notification-preferences')
    async updateAINotificationPreferences(
        @Body() preferences: AINotificationPreferences,
        @Res() response: Response
    ): Promise<void> {
        try {
            const accountToken = response.req.signedCookies['account'];
            ...
            const userId = this.authService.verifyAccountToken(accountToken);
            const updatedPreferences = await this.usersService.updateAINotificationPreferences(
                userId,
                preferences
            );
