import { Controller, Post, Delete, Get, Body, Param, Patch, HttpException, HttpStatus, Res, UseGuards } from '@nestjs/common';
...
export class WishlistController {
     ...
     */
    @Post('contact')
    async saveContact(@Res() response: Response, @Body() body: SaveContactDto): Promise<void> {
     ...
     */
    @Delete('contact/:id')
    async removeSavedContact(@Res() response: Response, @Param('id') contactId: string): Promise<void> {
        const userId = this.extractUserId(response);
        if (!userId) {
            response.status(HttpStatus.UNAUTHORIZED).json({
                statusCode: HttpStatus.UNAUTHORIZED,
        ...
        try {
            const result = await this.wishlistService.removeSavedContact(userId, contactId);
            response.json({
                statusCode: HttpStatus.OK,
                ...result,
            });
    ...
    async updateContactNotes(
        ...
        try {
            const result = await this.wishlistService.updateContactNotes(userId, contactId, body.notes);
            response.json({
                statusCode: HttpStatus.OK,
