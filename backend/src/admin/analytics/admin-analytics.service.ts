    ...
    return this.cacheService.getOrSet(
      ...
      async () => {
        try {
          ...
          const completedTradesInPeriod = await this.tradeModel
            .find({
              tradePhase: 'COMPLETED',
              completedAt: {
          ...
          const prevCompletedTrades = await this.tradeModel
            .find({
              tradePhase: 'COMPLETED',
    ...
    return this.cacheService.getOrSet(
      ...
      async () => {
        try {
          ...
          // Time series
          const timeSeriesMap: Map<string, { count: number; value: number }> =
            new Map();
          ...
          const timeSeries: TradeTimeSeries[] = Array.from(
          ...
          for (const phase of phases) {
            const count = await this.tradeModel.countDocuments({
              tradePhase: phase,
            });
          ...
          for (const [phaseName, [startField, endField]] of Object.entries(
            phaseTimestampFields,
          )) {
            const tradesWithBothDates = await this.tradeModel
              .find({
            ...
            if (tradesWithBothDates.length > 0) {
              ...
              phaseTimings.push({
                phase: phaseName,
                avgDays:
                  Math.round((totalDays / tradesWithBothDates.length) * 10) /
                  10,
              });
            } else {
          ...
          // Rejection reasons
          const rejectedTrades = await this.tradeModel
            .find({
          ...
          const rejectionReasons: RejectionReason[] = Array.from(
          ...
          const completedTradesCount = await this.tradeModel.countDocuments({
            tradePhase: 'COMPLETED',
          });
          const completionRate =
            totalTradesCount > 0
              ? Math.round((completedTradesCount / totalTradesCount) * 1000) /
                10
              : 0;
      ...
      async () => {
        try {
          ...
          for (const user of users) {
            ...
            registrationMap.set(
              dateKey,
              (registrationMap.get(dateKey) || 0) + 1,
            );
          }

          const registrations: UserRegistration[] = Array.from(
          ...
          const geographic: GeographicDistribution[] = Array.from(
          ...
          const step1 = companies.filter(
            (c) => c.onboardingProgress >= 25,
          ).length;
          const step2 = companies.filter(
            (c) => c.onboardingProgress >= 50,
          ).length;
          const step3 = companies.filter(
            (c) => c.onboardingProgress >= 75,
          ).length;
          ...
          const onboardingFunnel: OnboardingFunnelStep[] = [
            { step: 'Started', count: totalCompanies, dropoff: 0 },
            {
              ...
              dropoff:
                totalCompanies > 0
                  ? Math.round(
                      ((totalCompanies - step1) / totalCompanies) * 100,
                    )
                  : 0,
            },
            {
              ...
              count: step2,
              dropoff:
                step1 > 0 ? Math.round(((step1 - step2) / step1) * 100) : 0,
            },
            {
              step: 'Verification',
              count: step3,
              dropoff:
                step2 > 0 ? Math.round(((step2 - step3) / step2) * 100) : 0,
            },
            {
              step: 'Completed',
              count: step4,
              dropoff:
                step3 > 0 ? Math.round(((step3 - step4) / step3) * 100) : 0,
            },
          ];
      ...
      async () => {
        try {
          ...
          for (const trade of completedTrades) {
            ...
            valueMap.set(
              dateKey,
              (valueMap.get(dateKey) || 0) + price * quantity,
            );
          }

          const valueTimeSeries: ValueTimeSeries[] = Array.from(
            valueMap.entries(),
          )
          ...
          const prevAvgDealSize =
            prevCompletedTrades.length > 0
              ? Math.round(prevTotalValue / prevCompletedTrades.length)
              : 0;

          const avgDealSizeChange = this.calculatePercentageChange(
            avgDealSize,
            prevAvgDealSize,
          );
          ...
          for (const trade of completedTrades) {
            ...
            const category =
              product?.category || product?.productType || 'Uncategorized';
            const price =
              parseFloat(
                trade.buyerOfferedPrice?.toString() ||
                  trade.sellerOfferedPrice?.toString() ||
            ...
            categoryMap.set(
              category,
              (categoryMap.get(category) || 0) + price * quantity,
            );
          }
          ...
          // By incoterm
          const incotermMap: Map<string, { value: number; count: number }> =
            new Map();
          for (const trade of completedTrades) {
            const incoterm =
              ...
              'Unknown';
            const existing = incotermMap.get(incoterm) || {
              value: 0,
              count: 0,
            };
            const price =
        ...
        try {
          ...
          for (const company of companies) {
            ...
            for (const doc of pendingDocs) {
              totalPendingDocs++;
              const waitDays =
                (Date.now() - new Date(doc.uploadedAt).getTime()) /
                (1000 * 60 * 60 * 24);
          ...
          const kycBacklog: KycBacklog = {
            ...
            avgWaitDays:
              totalPendingDocs > 0
                ? Math.round((totalWaitDays / totalPendingDocs) * 10) / 10
                : 0,
          };
          ...
          const approvedDocs = await this.companyModel
            .find({
          ...
          for (const company of approvedDocs) {
            const approved = (company.kycDocuments || []).filter(
              (doc) =>
                doc.status === KycDocumentStatus.APPROVED && doc.reviewedAt,
            );
          ...
          documentProcessing.avgHours =
            processedDocsCount > 0
              ? Math.round((totalProcessingHours / processedDocsCount) * 10) /
                10
              : 0;
