...
export class AdminCompaniesService {
   ...
   */
  async getCompanies(query: GetCompaniesQueryDto): Promise<PaginatedCompaniesResult> {
    const {
      page = 1,
      limit = 20,
      search,
    ...
    const companiesWithStats = await Promise.all(
      companies.map(async (company) => {
        const userCount = await this.userModel.countDocuments({ company: company._id });
        const kycDocuments = company.kycDocuments || [];
        const pendingDocs = kycDocuments.filter((d: any) => d.status === KycDocumentStatus.PENDING).length;
  ...
  async deleteCompany(
    ...
    const companyUsers = await this.userModel.find({ company: companyId }).lean();
    ...
    const usersDeleted = await this.userModel.deleteMany({ company: companyId });
    ...
    await this.activityLogService.log({
      ...
      metadata: {
        cascadeResults: {
          ...
          tradesMarkedDeleted: buyerTradesUpdated.modifiedCount + sellerTradesUpdated.modifiedCount,
  ...
  async verifyCompany(
    ...
    for (const user of users) {
      await this.notificationService.createNotification({
        ...
        message: 'Congratulations! Your company has been verified. You now have the verified company badge.',
        priority: 'high',
      });
  ...
  async getCompanyStats(companyId: string): Promise<CompanyStats> {
    ...
    const [totalTrades, activeTrades, completedTrades, totalUsers] = await Promise.all([
      this.tradeModel.countDocuments({
        $or: [{ buyer: companyId }, { seller: companyId }],
      }),
      this.tradeModel.countDocuments({
  ...
  async getPageStats(): Promise<CompanyPageStats> {
    ...
    const [total, verified, newThisMonth] = await Promise.all([
      ...
      this.companyModel.countDocuments({ createdAt: { $gte: startOfThisMonth } }),
    ]);
    ...
    const [
      ...
      this.companyModel.countDocuments({ createdAt: { $lt: startOfThisMonth } }),
      this.companyModel.countDocuments({
        isKycVerified: true,
