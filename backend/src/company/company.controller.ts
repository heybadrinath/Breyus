import { Controller, Get, Post, Put, Delete, Body, Param, Res, HttpStatus, UseInterceptors, UploadedFiles, UseGuards } from '@nestjs/common';
...
export class CompanyController {
    ...
    async uploadKycDocument(
        @UploadedFiles() files: Express.Multer.File[],
        @Body() body: { documentType: string; customName: string },
        @Res() response: Response
    ): Promise<void> {
        try {
            const accountToken = response.req.signedCookies['account'];
            ...
            const validTypes = Object.values(KycDocumentType);
            if (!body.documentType || !validTypes.includes(body.documentType as KycDocumentType)) {
                response.status(HttpStatus.BAD_REQUEST).send({
                    statusCode: HttpStatus.BAD_REQUEST,
            ...
            const filePath = await this.storageService.upload(file.buffer, filename, 'kyc-documents');
            const fileUrl = this.storageService.getUrl(filePath);
    ...
    async uploadProfilePicture(
        ...
        try {
            ...
            const filePath = await this.storageService.upload(file.buffer, filename, 'profile-pictures');
            const fileUrl = this.storageService.getUrl(filePath);
    ...
    async uploadBanner(
        ...
        try {
            ...
            const filePath = await this.storageService.upload(file.buffer, filename, 'banners');
            const fileUrl = this.storageService.getUrl(filePath);
