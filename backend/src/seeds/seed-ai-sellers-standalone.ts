...
// MongoDB connection - NEVER commit credentials to source control
// Use environment variables in production/development
const MONGO_URI =
  process.env.MONGODB_URI_DEV ||
  process.env.MONGODB_URI ||
  'mongodb://localhost:27017/breyus';

// Note: For Docker with auth, set MONGODB_URI_DEV environment variable
  ...
  for (const seller of seedSellers) {
    try {
      ...
      const company = await Company.create({
        ...
        websiteUrl: `https://${seller.companyName
          .toLowerCase()
          .replace(/\s+/g, '')
      ...
      console.error(
        `[ERROR] Failed to create ${seller.companyName}:`,
        error.message,
      );
    }
  }
