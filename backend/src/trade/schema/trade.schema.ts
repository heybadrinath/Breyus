...
// ========================
// DOCUMENT REJECTION TRACKING (Phase 2 Refactoring)
// ========================
// Tracks rejection attempts per document type to enforce upload limits
// Auto-cancellation triggers when maxAttempts is exceeded
export interface DocumentRejectionTracking {
  rejectionCount: number; // Number of times this document has been rejected
  lastRejectionAt?: Date; // When the document was last rejected
  lastRejectionReason?: string; // Reason for the last rejection
  maxAttempts: number; // Maximum upload attempts allowed (2 for most, 3 for BoL)
}
...
export interface NegotiationEntry {
  round: number;
  party: 'buyer' | 'seller';
  offeredPrice?: number | string; // FIXED: Accepts both for backwards compatibility with existing data
...
// SPA Document info - now uses approval flow instead of dual signatures
...
export interface SPADocumentInfo extends DocumentInfo {
  // DEPRECATED: Signature fields removed in Phase 2 refactoring
  // Keeping for backwards compatibility during migration
  sellerSignatureDataUrl?: string; // @deprecated - use signedSpaDocument instead
  sellerSignedAt?: Date; // @deprecated
  sellerSignedBy?: Types.ObjectId; // @deprecated
  buyerSignatureDataUrl?: string; // @deprecated - use signedSpaDocument instead
  buyerSignedAt?: Date; // @deprecated
  buyerSignedBy?: Types.ObjectId; // @deprecated
}
...
export class Trade extends Document {
  ...
  // Nearest importing port (can be selected from dropdown or custom)
  @Prop()
  nearestPort?: string;

  // Buyer's CIS (Customer Information Sheet) document path
  // Can be from company profile or uploaded during PR creation
  @Prop()
  ...
  // FIXED: Changed from string to number to match buyerOfferedPrice (Schema consistency fix)
  @Prop({ type: Number, min: 0 })
  ...
  // ========================
  // NEGOTIATION COUNTER TRACKING (Counter Limit Feature)
  // ========================
  // Limits buyer to 2 re-counters, after which seller must Accept/Reject

  // Number of times buyer has submitted a counter/response
  @Prop({ type: Number, default: 0 })
  buyerCounterCount: number;

  // Maximum allowed buyer counters (default: 2)
  @Prop({ type: Number, default: 2 })
  maxBuyerCounters: number;

  // True when buyer has exhausted all counter opportunities
  @Prop({ type: Boolean, default: false })
  isNegotiationLocked: boolean;

  // When the "last counter warning" email was sent to buyer
  @Prop()
  lastCounterWarningAt?: Date;
  ...
  // ========================
  // SIGNED SPA (Phase 2 Refactoring)
  // ========================
  // Buyer's signed SPA document (separate from seller's SPA)
  // Flow: Seller SPA approved → Buyer uploads signed SPA → Seller approves

  @Prop({ type: Object })
  signedSpaDocument?: DocumentInfo;

  @Prop()
  signedSpaSubmittedAt?: Date;

  @Prop()
  signedSpaApprovedAt?: Date;

  // ========================
  // DOCUMENT REJECTION TRACKING (Phase 2 Refactoring)
  // ========================
  // Tracks rejection attempts per document to enforce upload limits

  @Prop({
    type: Object,
    default: { rejectionCount: 0, maxAttempts: 2 },
  })
  ...
  @Prop({
    type: Object,
    default: { rejectionCount: 0, maxAttempts: 3 }, // BoL gets 3 attempts
  })
  ...
  // ========================
  // AUTO-CANCELLATION (Phase 2 Refactoring)
  // ========================
  // Tracks trades auto-cancelled due to document rejection limits
  ...
  // ========================
  // DISPUTE ELIGIBILITY (Phase 2 Refactoring)
  // ========================
  // 30-day window after completion/cancellation for raising disputes

  @Prop()
  disputeEligibilityEndsAt?: Date;
...
// Phase 2 Refactoring indexes
TradeSchema.index({ cancelledAt: 1, tradePhase: 1 }); // Cancelled trade visibility (2-day filter)
TradeSchema.index({ disputeEligibilityEndsAt: 1 }); // Dispute window expiry queries
TradeSchema.index({ autoCancelledAt: 1 }); // Auto-cancelled trade tracking

// Negotiation counter limit indexes
TradeSchema.index({ isNegotiationLocked: 1, negotiationStatus: 1 }); // Locked negotiation queries
