...
export class NotificationController {
  ...
  async getRecentUnread(
    ...
    try {
      ...
      response
        .status(HttpStatus.OK)
        .json({ statusCode: HttpStatus.OK, data: notifications });
    } catch (error) {
      const status = error.status || HttpStatus.INTERNAL_SERVER_ERROR;
      response
        .status(status)
  ...
  async getUnreadCount(
    ...
    try {
      ...
      response
        .status(HttpStatus.OK)
        .json({ statusCode: HttpStatus.OK, data: { count } });
    } catch (error) {
      const status = error.status || HttpStatus.INTERNAL_SERVER_ERROR;
      response
        .status(status)
   ...
   */
  @Get('unread-count-breakdown')
  async getUnreadCountBreakdown(@Res() response: Response) {
    try {
      const userId = this.getUserIdFromCookie(response.req);
      const breakdown =
      ...
      response
        .status(HttpStatus.OK)
        .json({ statusCode: HttpStatus.OK, data: breakdown });
    } catch (error) {
      const status = error.status || HttpStatus.INTERNAL_SERVER_ERROR;
      response
        .status(status)
  ...
  async deleteNotification(@Res() response: Response, @Param('id') id: string) {
    try {
      ...
      response
        .status(HttpStatus.OK)
        .json({ statusCode: HttpStatus.OK, message: 'Notification deleted' });
    } catch (error) {
      const status = error.status || HttpStatus.INTERNAL_SERVER_ERROR;
      response
        .status(status)
