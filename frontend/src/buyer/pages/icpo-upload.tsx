...
import { uploadICPO, getTradeById, Trade } from "../../services/trade.service";
...
const TradeProgressBar = ({ currentStep }: { currentStep: number }) => {
    const steps = [
        ...
        { label: "Purchase Order", step: 2 },
        { label: "Sale and Purchase Agreement", step: 3 },
        { label: "Bill of lading", step: 4 },
    ];
...
interface TradeWithProduct extends Omit<Trade, 'purchaseOrderStatus' | 'purchaseRequestStatus' | 'negotiationStatus' | 'seller'> {
    product: {
        _id: string;
        name: string;
        price: string;
...
const getProgressStep = (tradePhase?: string): number => {
    switch (tradePhase) {
        ...
        case 'COMPLETED':
            return 4;
        default:
            return 2; // Default to step 2 for ICPO upload
    }
};

export const ICPOUpload = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            setUploading(true);
            setError(null);
            await uploadICPO(tradeId, selectedFile);
            navigate("/buyer/trade?tab=po", {
                state: { success: "ICPO uploaded successfully!" }
            });
        } catch (err: any) {
            setError(err.message || "Failed to upload ICPO");
        } finally {
            setUploading(false);
        }
    };
    ...
    // Check if ICPO is already uploaded
    const hasExistingICPO = trade?.icpoDocument && trade.icpoDocument.filePath && !replaceMode;
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <div className="flex-1 p-8 max-w-5xl mx-auto w-full">
                {/* Back Button */}
                <button
                    onClick={() => navigate("/buyer/trade?tab=po")}
                ...
                <h1 className="text-2xl font-bold text-gray-900 mb-6">
                    Your Purchase Order Details (ICPO)
                </h1>
                    ...
                    <div className="bg-white rounded-lg border p-4 mb-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-sm font-medium">{trade.seller?.mail || 'N/A'}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {hasExistingICPO ? (
                    <div className="bg-white rounded-lg border p-8 mb-6">
                        ...
                        <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-lg mb-6">
                            ...
                            <div>
                                <h3 className="font-semibold text-green-800">ICPO Document Uploaded</h3>
                                <p className="text-sm text-green-700">Your purchase order has been sent to the seller</p>
                            </div>
                        </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    <p className="font-medium text-gray-900">
                                        {trade.icpoDocument?.originalName || 'ICPO Document'}
                                    </p>
                                    <div className="flex items-center gap-4 text-sm text-gray-500 mt-1">
                                        {trade.icpoDocument?.uploadedAt && (
                                            <span>
                                                Uploaded: {new Date(trade.icpoDocument.uploadedAt).toLocaleDateString('en-US', {
                                                    month: 'short',
                                                    day: 'numeric',
                                                    year: 'numeric',
                                                    hour: '2-digit',
                                        ...
                                        {trade.icpoDocument?.version && trade.icpoDocument.version > 1 && (
                                            <span className="text-blue-600">Version {trade.icpoDocument.version}</span>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex items-center gap-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg mb-6">
                            ...
                            <span className="text-sm text-yellow-800">
                                {trade.icpoDocument?.status === 'approved'
                                    ? 'ICPO verified by seller - Ready for SPA'
                                    : trade.icpoDocument?.status === 'rejected'
                                    ? 'ICPO rejected - Please upload a new document'
                        ...
                        <div className="flex gap-3">
                            <a
                                href={getDocumentUrl(trade.icpoDocument!.filePath)}
                                target="_blank"
                            ...
                            <a
                                href={getDocumentUrl(trade.icpoDocument!.filePath)}
                                download={trade.icpoDocument?.originalName || 'ICPO'}
                                className="flex-1 px-4 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 flex items-center justify-center gap-2 text-gray-700"
                            >
                    ...
                    <>
                            ...
                            {!selectedFile ? (
                                <div className="flex flex-col items-center justify-center min-h-[300px]">
                                    ...
                                    <p className="text-lg text-gray-600 mb-2">
                                        {replaceMode ? 'Upload new ICPO document' : 'Drag and drop your ICPO document here'}
                                    </p>
                        ...
                        <div className="flex items-start gap-3 mb-6">
                            ...
                            <label htmlFor="agreement" className="text-sm text-gray-600">
                                I agree to all the details present in above document
                            </label>
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-500 mb-8 bg-gray-100 px-4 py-3 rounded-lg">
                            ...
                            <span>Your CIS (Customer Information Sheet) is attached default for this Purchase Order</span>
                        </div>
                        ...
                        <div className="flex justify-end gap-4">
                            ...
                            >
                                {uploading ? (
                                    ...
                                    replaceMode ? 'Replace ICPO Document' : 'Send ICPO (Purchase Order)'
                                )}
                            </button>
                        </div>
                    </>
            ...
            {trade && (
                <DocumentReplaceModal
                    ...
                    documentType="icpo"
                    currentDocument={trade.icpoDocument}
                />
            )}
        </div>
    );
