...
const Tab: React.FC<{ label: string; isActive: boolean; onClick: () => void }> = ({ label, isActive, onClick }) => {
  return (
    <div
      onClick={onClick}
      className={`cursor-pointer px-4 py-2 text-sm font-semibold transition-all duration-300 ease-in-out ${isActive ? 'text-black border-b-[3px] font-extrabold text-[1.12rem] border-gray-300' : 'text-gray-400 text-[1.1rem]'
        }`}
    >
      {label}
    </div>
...
const Tabs: React.FC<{ tabs: string[]; activeTab: string; onTabChange: (tab: string) => void }> = ({ tabs, activeTab, onTabChange }) => {
  return (
    <div className="flex justify-evenly w-[98%] mx-auto border-b-[2px] border-gray-200">
      {tabs.map((tab) => (
        <Tab
          key={tab}
          label={tab}
