...
const Incoterms: React.FC<IncotermsProps> = ({incoterms, setIncoterms}) => {
  ...
  const FlipableButton = ({ 
    ...
    return (
      <button
        disabled={disabled}
        className={` !w-full  ${(!disabled) ? 'cursor-pointer hover:scale-[1.1]' : 'cursor-not-allowed'} py-6 flex transition-all delay-100 ease-in-out ${(currentValue === 'Buyer') ? 'bg-[#EDE0D3]' : (currentValue === 'Seller') ? 'bg-[#71DE5F]' : 'bg-[#FFBB00]'}`}
        onClick={handleClick}>
        <p className='mx-auto flex'>{currentValue}
