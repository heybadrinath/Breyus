...
const InboxConversation: React.FC<InboxConversationProps> = ({ name, productName, messages, onSendMessage, isTyping, onTyping }) => {
    ...
    const handleSendMessage = () => {
        if (message.trim()) {
            console.log("Sending message from InboxConversation:", message);
        ...
        <div className="flex-1 flex flex-col bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-5 bg-gray-50">
                {messages.map((message) => (
                    ...
                    >
                        <div className={`max-w-[70%] p-3.5 rounded-2xl shadow-sm relative ${message.isSender ? "bg-gray-600 text-white" : "bg-white text-gray-900"}`}>
                            <div className="text-base leading-tight break-words">{message.text}</div>
