 ...
 *
 * Features:
 * - Twitter/X, LinkedIn, WhatsApp, Email
 * - Copy link with feedback
 * - Records share event for analytics
 */
export function ShareButtons({ postId, title, url }: ShareButtonsProps) {
  ...
  const handleShare = async (platform: string, shareUrl: string) => {
    ...
    try {
      await blogPostsService.recordShare(postId);
    } catch {
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <button
        ...
        className="p-2 rounded-lg text-gray-500 hover:text-blue-500 hover:bg-blue-50 transition-colors"
