...
const ProductProgressVector = ({ page }: { page: string }) => {
    // Calculate fill percentage based on current page
    let fillPercentage = 0;
    if (page === "product") fillPercentage = 0;
    else if (page === "media") fillPercentage = 33.3;
    else if (page === "price") fillPercentage = 66.7;
    else if (page === "tags") fillPercentage = 100;
    
    // Define steps
    const steps = [
        { id: "product", label: "Product", active: ["product", "media", "price", "tags"].includes(page) },
        { id: "media", label: "Media", active: ["media", "price", "tags"].includes(page) },
        { id: "price", label: "Price", active: ["price", "tags"].includes(page) },
        { id: "tags", label: "Tags", active: ["tags"].includes(page) }
    ];
            ...
            <div className="steps-container">
                {steps.map((step) => (
                    <div key={step.id} className="progress-step">
                        <div className={`progress-dot ${step.active ? 'active' : ''}`}>
                            <div className="dot-inner" />
                        </div>
                        <div className={`progress-label ${step.active ? 'active' : ''}`}>
                            {step.label}
                        </div>
                    </div>
                ))}
...
const ProductLayout = ({ productype, Body }: { productype: string, Body: ReactNode }) => {
    return (
        <div className="max-w-4xl mx-auto my-14 flex flex-col">
            <ProductProgressVector page={productype} />
            <div className="product-layout-container py-8 px-6 mt-4">
                {Body}
            </div>
        </div>
    );
