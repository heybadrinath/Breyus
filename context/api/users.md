...
Retrieves the email of the authenticated user. Despite the endpoint name, this returns the user's `mail` field (email address), not a display name.
...
> **Note:** The response field is called `name` but actually contains the user's email address (`user.mail`). This is a legacy naming quirk.
...
- Returns `user.mail` (the user's email address) in the `name` field
- Used for displaying user identifier in UI
...
1. **Display User Identifier:**
   ```javascript
   const fetchUserEmail = async () => {
     // Note: response field is called "name" but contains the user's email
     const { name: userEmail } = await api.post('/users/return-name');
