...
type: operations-doc
module: operations
tags: [operations, capacity_planning]
---
...
**Infrastructure**: DigitalOcean Droplet Singapore (8 vCPU, 16 GB RAM, 320 GB NVMe) @ $112/month + DigitalOcean Spaces @ $5/month

---

## Executive Summary

✅ **CAPACITY VERDICT**: DigitalOcean Droplet will comfortably handle 2k users (100-200 concurrent)
...
✅ **LATENCY**: Singapore datacenter provides 50-80ms latency to India/South Asia users (vs 200ms+ from Europe)
✅ **SCALING THRESHOLD**: Can stretch to 5k users with optimizations, 10k+ requires horizontal scaling
✅ **COST**: $0.059 per user/month at 2,000 users
...
> **Note:** These figures are **projected estimates**, not measured from load tests. Actual numbers may differ — conduct load testing before production launch.

**Average Load** (typical business hours, 100 concurrent users / 5% concurrency):
- Buyer Sessions: 50 users × 65 API calls/hour = 3,250 calls/hour
- Seller Sessions: 50 users × 50 API calls/hour = 2,500 calls/hour
- **Total Average: ~5,750 API calls/hour** (~1.6 calls/second)

**Peak Load** (end-of-day rush, 200 concurrent users / 10% concurrency):
- Buyer Peak: 100 × 140 API calls/hour = 14,000 calls/hour
- Seller Peak: 100 × 80 API calls/hour = 8,000 calls/hour
- **Peak Total: ~22,000 API calls/hour** (~6 calls/second)

**Theoretical Maximum** (all 1,000 active users online — unlikely):
- Buyer Sessions: 500 × 140 = 70,000 calls/hour
- Seller Sessions: 500 × 80 = 40,000 calls/hour
- **Theoretical Max: 110,000 API calls/hour** (~31 calls/second)
...
### File Storage (DigitalOcean Spaces - 1 TB Standard Tier)
...
### DigitalOcean Droplet Singapore (8 vCPU, 16 GB RAM, 320 GB NVMe) - All Services via Docker Compose
...
| **Available** | **8** | **16 GB** | **160 GB** | DigitalOcean Premium Droplet capacity |
| **Buffer** | **2 (25%)** | **1.5 GB (9%)** | **50 GB** | ⚠️ **RAM TIGHT** |

> **RAM Warning:** 9% RAM buffer (1.5 GB free) is tight. Linux needs ~500MB–1GB for OS page cache and system processes. A Node.js memory spike or PyTorch model reload could trigger OOM killer. **Mitigations:** (1) Configure 4GB swap file as safety net (see [[operations/SERVER_SETUP#Swap Configuration]]). (2) Set Docker memory limits per container to prevent runaway processes. (3) If RAM pressure becomes an issue, upgrade to 32GB droplet (~$168/mo).
>
> **Disk Note:** The "< 10 GB" figure from the initial audit was incorrect — PostgreSQL uses 38GB of an 80GB allocation, leaving ~42GB free within the allocated partition. Total NVMe is 320GB with 160GB available after all allocations.
...
- **Still runs on single DigitalOcean Droplet**
...
- **Estimated Cost**: $200-250/month (2-3× Droplets + managed MongoDB)
...
|------|--------------|-------------|
| DigitalOcean Droplet Singapore (8 vCPU, 16 GB, 320 GB NVMe) | $112.00 | $1,344.00 |
| DigitalOcean Spaces Standard (250 GB) | $5.00 | $60.00 |
| Domain + SSL (Let's Encrypt) | ~$1.00 | ~$12.00 |
| **Total** | **$118.00** | **$1,416.00** |

**Cost per User**: $0.059/month (2,000 users)

### Why DigitalOcean Over Hetzner?

| Aspect | Hetzner (Previous) | DigitalOcean Singapore (Current) |
|--------|-------------------|------------------------|
| **Monthly Cost** | ~$42 | $112 |
| **Latency to India** | 200-300ms | 50-80ms |
| **Storage Type** | SATA SSD + CIFS mount | NVMe SSD + S3 API |
| **Datacenter** | Germany | Singapore, India |
| **Uptime SLA** | 99.9% | 99.99% |

**Trade-off**: +$70/month for **3-5× better latency** and **Singapore datacenter** for Indian users.
...
**DigitalOcean (Singapore — Our Choice)**:
- Droplet (8 vCPU, 16 GB): $112/month
...
**Verdict**: DigitalOcean offers **best value for India** with Singapore datacenter at $117/month ✅
...
- **Latency**: Singapore datacenter provides excellent India coverage

### Medium Risk ⚠️
- **Single Point of Failure**: No redundancy (acceptable for MVP)
- **Backup Strategy**: Need automated backups to Object Storage bucket
- **DDoS Protection**: DigitalOcean offers optional DDoS protection (+$10/month) or use CloudFlare Free tier
...
1. **DigitalOcean Automated Backups**: +$22/month (20% of server cost, essential for production)
...
4. **Load Balancer**: Not needed until 5k+ users ($12-24/month when required)
...
**Reference**: See Admin Portal source at `admin-portal/src/features/system/` for implementation details.
...
> **Critical:** If the server is down, the Admin Portal monitoring is also down. External monitoring is not optional — it's the only way to know about outages.

**Required (Free):**
- **Uptime Robot** or **Better Stack** (free tier): Monitor `https://breyus.com`, `https://api.breyus.com/health`, `https://admin.breyus.com` — alerts via email/Slack when down
- **DigitalOcean Monitoring** (free, built-in): CPU, RAM, disk, bandwidth alerts — enable in Droplet settings
...
**Database Backups**:
- **MongoDB**: Daily automated dumps (14-day retention) — see [[operations/BACKUP_AND_RECOVERY]]
- **PostgreSQL**: Weekly versioned backups (4-copy / 28-day retention)
- **Storage**: Local Docker volume + upload to DigitalOcean Spaces (`breyus-files/backups/`)
...
**File Storage**:
- **DigitalOcean Spaces**: Primary object storage (S3-compatible, auto-redundant)
...
**Your DigitalOcean Singapore deployment plan is well-architected and will comfortably handle 2,000 users (100-200 concurrent) with excellent latency for Indian users.**

### Key Strengths
- **Latency**: Singapore datacenter provides 50-80ms latency to India/South Asia (3-5× better than Europe)
- **Headroom**: 25% CPU buffer and 9% RAM buffer under peak load
- **Storage**: 1 TB Object Storage with 25% unused after 12 months
- **Cost Model**: Fixed monthly pricing eliminates surprise bills ($117/month total)
- **Scalability**: Can stretch to 5k users on same infrastructure with optimizations
- **Efficiency**: $0.059 per user/month at 2,000 users
...
- **File Storage**: 744 GB/year usage vs 1 TB capacity (can upgrade when needed)
- **Concurrent Connections**: 260-300 WebSockets (manageable)
- **AI Requests**: 200-400/day with 90% cache hit rate (low load)

### Scaling Milestones
- **0-2k users**: Single DigitalOcean Droplet Singapore (current plan) ✅
- **2k-5k users**: Same server with performance optimizations ✅
- **5k-10k users**: Add additional DigitalOcean instances + DigitalOcean Load Balancer (~$200-250/month)
- **10k+ users**: Managed MongoDB Atlas + multi-region setup (~$500+/month)

### Recommendation
Your DigitalOcean Singapore infrastructure is production-ready for the target user base with excellent latency for Indian users. The 320 GB NVMe SSD provides sufficient space for databases, and 1 TB Object Storage handles all file uploads with room to grow.

**Optional Enhancements:**
- Add CloudFlare Free tier for DDoS protection and CDN caching
- Enable DigitalOcean automated backups (+20% server cost = ~$22/month)
- Consider DigitalOcean DDoS protection for production (+$10/month)

---

## Appendix: Performance Benchmarks (Projected)

> **These benchmarks are projected estimates**, not from real load tests. Conduct load testing with tools like `k6`, `wrk`, or `artillery` before production launch to validate these numbers.
...
### Concurrent User Capacity Projections

| Concurrent Users | API Load (req/sec) | CPU Usage (est.) | RAM Usage (est.) | Response Time (P95 est.) | Status |
|------------------|-------------------|-----------|-----------|---------------------|--------|
...
**Infrastructure**: DigitalOcean Singapore High Performance + Object Storage
