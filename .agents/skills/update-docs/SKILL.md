...
description: >-
  ...
  MOC index files, AGENTS.md Feature Status, SESSION_CONTEXT.md, and wikilinks.
...
- `AGENTS.md` — Feature Status section (single source of truth)
...
3. **Update changelogs** (if area is `changelog` or `all`)
   ...
   - Include YAML frontmatter: `type: changelog`, `date`, `change_type`, `scope`, `tags`
   - Add Summary, Changes, Files Modified, and Related sections
...
5. **Update Feature Status in AGENTS.md** (if area is `feature-status` or `all`)
   - Read the current Feature Status section in `AGENTS.md`
   ...
   - AGENTS.md Feature Status is the SINGLE SOURCE OF TRUTH — be accurate
